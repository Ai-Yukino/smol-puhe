...
model1 <- glm(gender~f0, data = tb)
summary(model1)

## Logistic regression ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## gender ~ f0 / f1

tb$quotient1 <- tb$f0 / tb$f1

ggplot(tb, aes(x=quotient1, y=gender)) +
  geom_point(alpha=.5, aes(color = cut(tb$gender, c(-1, 0.5, 1)))) +
  scale_color_manual(name = "Gender",
                     values = c("(-1,0.5]" = "#ff0080",
                                "(0.5,1]" = "#0000ff"),
...
model1 <- glm(gender~quotient1, data = tb)
summary(model1)

## Logistic regression ########################################################
## gender ~ f1 / f0

tb$quotient2 = tb$f1 / tb$f0

ggplot(tb, aes(x=quotient2, y=gender)) +
  geom_point(alpha=.5, aes(color = cut(tb$gender, c(-1, 0.5, 1)))) +
  scale_color_manual(name = "Gender",
                     values = c("(-1,0.5]" = "#ff0080",
                                "(0.5,1]" = "#0000ff"),
...
model1 <- glm(gender~quotient2, data = tb)
summary(model1)

## Logistic regression ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## gender ~ f1 - f0

tb$difference = tb$f1 - tb$f0

ggplot(tb, aes(x=difference, y=gender)) +
  geom_point(alpha=.5, aes(color = cut(tb$gender, c(-1, 0.5, 1)))) +
  scale_color_manual(name = "Gender",
                     values = c("(-1,0.5]" = "#ff0080",
                                "(0.5,1]" = "#0000ff"),
...
model1 <- glm(gender~difference, data = tb)
summary(model1)
